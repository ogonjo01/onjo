...
const PERIOD_MS   = { Today: 864e5, Week: 6048e5, Month: 2592e6, Year: 3154e7 };
const PERIOD_TRUNC = { Today: 'hour', Week: 'day', Month: 'day', Year: 'month' };
...
const AnalyticsDashboard = ({ theme: T }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Top content by views this period
      ...
      setCat(Object.entries(catMap).sort((a, b) => b[1] - a[1]).map(([name, value], i) => ({
        name, value: Math.round(value / total * 100), color: CAT_COLOURS[i % CAT_COLOURS.length],
      })));
  ...
  const getDrillContent = () => {
    if (period === 'Week' && selectedDay !== null) {
      const dc = {};
      rawViews.forEach(r => {
        if (new Date(r.created_at).getDay() === selectedDay) dc[r.post_id] = (dc[r.post_id] || 0) + 1;
      });
  ...
  return (
    <div style={{ overflowY: 'auto', flex: 1, paddingRight: 4, paddingBottom: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        {METRICS.map(m => (
          <StatCard key={m.id} label={m.label} icon={m.icon} value={stats[m.id] ?? 0} delta={pct(stats[m.id] ?? 0, prev[m.id] ?? 0)} accent={m.color} theme={T} />
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 230px', gap: 10, marginBottom: 12 }}>
                ...
                <div key={c.id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 210 }}>
                        {verdictIcon && <span style={{ marginRight: 4 }}>{verdictIcon}</span>}{c.title}
                      </div>
        ...
        {/* Right column: categories + verdicts */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '14px 12px', flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            {catData.length > 6 && (
              <input value={catSearch} onChange={e => setCatSearch(e.target.value)} placeholder="Filter…" style={{
                width: '100%', padding: '5px 8px', marginBottom: 8, background: T.inputBg,
                border: `1px solid ${T.inputBorder}`, borderRadius: 6, color: T.text, fontSize: 11, outline: 'none', boxSizing: 'border-box',
              }} />
            )}
            <ResponsiveContainer width="100%" height={80}>
              <PieChart>
                <Pie data={catData} cx="50%" cy="50%" innerRadius={18} outerRadius={36} dataKey="value" paddingAngle={2} stroke="none">
              ...
              {filteredCats.map(c => (
                <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    ...
                    <span style={{ fontSize: 10, color: T.textSub, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 110 }}>{c.name}</span>
                  </div>
...
const MODE_MAP = { trending: 'trending', ideas: 'recommendations', news: 'news', seo: 'seo' };
...
const AIAdviser = ({ theme: T }) => {
  ...
  // Core fetch — passes history for multi-turn chat memory
  ...
  // Send a chat message with full history for memory
  const sendChat = async (override = '') => {
    const text = (override || chatInput).trim();
    if (!text || chatLoading) return;
    setChatInput('');
    const historySnapshot = [...messages]; // capture before appending
  ...
  // Fetch non-chat tab data
  const fetchTab = async (tab, topic = '') => {
    setLoading(true);
    setError(null);
    try {
      const text = await callAdviser(MODE_MAP[tab] || tab, '', topic || customTopic);
  ...
  // Jump to chat with a pre-filled question from another tab
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 5, marginBottom: 8, flexWrap: 'wrap', flexShrink: 0 }}>
        {ADVISER_PROMPTS[subTab].map((p, i) => (
          <button key={i}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexShrink: 0 }}>
            <input
              ...
              placeholder={PLACEHOLDER_MAP[subTab] || 'Type a product category or niche…'}
            ...
            <button onClick={() => fetchTab(subTab)} disabled={loading} style={{
              ...
              cursor: loading ? 'default' : 'pointer', fontSize: 12, fontWeight: 600, opacity: loading ? 0.7 : 1, flexShrink: 0,
            }}>{loading ? 'Searching…' : '↻ Search'}</button>
          </div>

          {error && (
            <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: '8px 12px', color: '#f87171', fontSize: 12, marginBottom: 8, flexShrink: 0 }}>
              ⚠️ {error}
            </div>
          )}
            ...
            {loading ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, gap: 12 }}>
                ...
                <span style={{ fontSize: 12, color: T.textMuted }}>
                  Searching for {subTab === 'trending' ? 'trending products' : subTab === 'ideas' ? 'review opportunities' : subTab === 'news' ? 'latest product news' : 'SEO insights'}…
                </span>
              </div>
            ) : currentResult ? (
              <div>
              ...
              <div style={{ textAlign: 'center', padding: '40px 16px', color: T.textMuted }}>
                ...
                <button onClick={() => fetchTab(subTab)} style={{ background: T.accentGrad, color: '#fff', border: 'none', borderRadius: 9, padding: '10px 24px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
            ...
            <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 180px', gap: 12, padding: '12px 16px', alignItems: 'center', borderBottom: i < users.length - 1 ? `1px solid ${T.border}` : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 10, color: T.textMuted }}>{new Date(u.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</div>
                </div>
              </div>
...
const ProfileEdit = ({ profile, onSaved, theme: T }) => {
  ...
  if (!editing) return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <button onClick={() => { setEditing(true); setInput(profile?.username || ''); }} style={{ background: T.accent + '18', border: `1px solid ${T.accent}44`, color: T.accent, padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
        Edit Username
      </button>
    </div>
  );
...
const UserProfile = ({ onClose, onUpdated }) => {
  ...
  if (showDash) {
    ...
    return (
      <div
        style={{ position: 'fixed', inset: 0, background: T.overlay, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1400, padding: '3%', boxSizing: 'border-box' }}
  ...
  // Non-dashboard (regular user) view
  return (
