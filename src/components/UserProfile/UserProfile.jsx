...
// ONJO Reviews — calls /api/review-adviser (NOT /api/ai-advisor which is OGONJO only)
...
const THEMES = {
  dark: {
    ...
    tooltipBg:'#0f172a', statCardBg:'rgba(255,255,255,0.03)', statCardBorder:'rgba(255,255,255,0.08)',
  },
  white: {
    ...
    tooltipBg:'#1e293b', statCardBg:'#fff', statCardBorder:'#e5e7eb',
  },
  green: {
    ...
    tooltipBg:'#022c22', statCardBg:'rgba(16,185,129,0.04)', statCardBorder:'rgba(16,185,129,0.12)',
  },
};
...
const VERDICT_COLOURS = { recommended:'#16a34a', not_recommended:'#dc2626', neutral:'#6b7280' };
...
const StatCard = ({ label, value, delta, accent, icon, theme:T }) => (
  <div style={{ background:T.statCardBg, border:`1px solid ${T.statCardBorder}`, borderRadius:12,
...
const AnalyticsDashboard = ({ theme:T }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true); setShowAll(false); setSelectedDay(null);
    try {
      ...
      const {data:pv}=await supabase.from('views').select('post_id,created_at').gte('created_at',sinceA).not('post_id','is',null);
      setRawViews(pv||[]);
      if(pv?.length){
        const vc={};
        (pv||[]).forEach(r=>{ vc[r.post_id]=(vc[r.post_id]||0)+1; });
        const topIds=Object.entries(vc).sort((a,b)=>b[1]-a[1]).map(([id])=>id);
        const {data:details}=await supabase.from('book_summaries')
          .select('id,title,category,views_count,likes_count,avg_rating,verdict').in('id',topIds);
        setTop((details||[]).map(c=>({...c,period_views:vc[c.id]||0})).sort((a,b)=>b.period_views-a.period_views));
      } else { setTop([]); }

      const {data:cats}=await supabase.from('book_summaries').select('category,views_count').not('category','is',null);
      const catMap={};
      ...
      const {data:verdicts}=await supabase.from('book_summaries').select('verdict').not('verdict','is',null);
      const vMap={};
      (verdicts||[]).forEach(r=>{ if(r.verdict) vMap[r.verdict]=(vMap[r.verdict]||0)+1; });
  ...
  const getDrillContent=()=>{
    if(period==='Week'&&selectedDay!==null){
      const dc={};
      rawViews.forEach(r=>{ if(new Date(r.created_at).getDay()===selectedDay) dc[r.post_id]=(dc[r.post_id]||0)+1; });
      const ids=new Set(Object.keys(dc));
      return topContent.filter(c=>ids.has(c.id)).map(c=>({...c,period_views:dc[c.id]||0})).sort((a,b)=>b.period_views-a.period_views);
    }
    return topContent;
  };
    ...
    <div style={{ overflowY:'auto', flex:1, paddingRight:4, paddingBottom:8 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <div style={{ display:'flex', gap:3, background:T.surface, borderRadius:8, padding:3, border:`1px solid ${T.border}` }}>
            {PERIODS.map(p=>(
              <button key={p} onClick={()=>setPeriod(p)} style={{ background:period===p?T.tabActive:'transparent', border:'none', color:period===p?T.tabText:T.tabInactive, padding:'4px 10px', borderRadius:6, cursor:'pointer', fontSize:11, fontWeight:600, transition:'all 0.15s' }}>{p}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 230px', gap:10, marginBottom:12 }}>
        <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:'14px 16px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
            <span style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1.5, color:T.textSub }}>Top Reviews — {period}</span>
            {drillContent.length>8&&<button onClick={()=>setShowAll(v=>!v)} style={{ background:'none', border:`1px solid ${T.border}`, borderRadius:20, color:T.accent, fontSize:10, fontWeight:700, cursor:'pointer', padding:'2px 10px' }}>{showAll?'▲ Show less':`View all ${drillContent.length} →`}</button>}
          </div>
          {period==='Week'&&daysWithViews.length>0&&(
            <div style={{ display:'flex', gap:4, marginBottom:10, flexWrap:'wrap' }}>
              <button onClick={()=>setSelectedDay(null)} style={{ padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:600, cursor:'pointer', background:selectedDay===null?T.accent:T.surface, color:selectedDay===null?'#fff':T.textMuted, border:`1px solid ${T.border}` }}>All days</button>
              {daysWithViews.map(d=><button key={d} onClick={()=>setSelectedDay(d)} style={{ padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:600, cursor:'pointer', background:selectedDay===d?T.accent:T.surface, color:selectedDay===d?'#fff':T.textMuted, border:`1px solid ${T.border}` }}>{DAY_NAMES[d].slice(0,3)}</button>)}
            </div>
          )}
                ...
                <div key={c.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 0', borderBottom:i<visibleContent.length-1?`1px solid ${T.border}`:'none' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
                    ...
                    <div style={{ minWidth:0 }}>
                      <div style={{ fontSize:11, fontWeight:600, color:T.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:210 }}>{verdictIcon&&<span style={{ marginRight:4 }}>{verdictIcon}</span>}{c.title}</div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:'14px 12px', flex:1, display:'flex', flexDirection:'column' }}>
            ...
            {catData.length>6&&<input value={catSearch} onChange={e=>setCatSearch(e.target.value)} placeholder="Filter…" style={{ width:'100%', padding:'5px 8px', marginBottom:8, background:T.inputBg, border:`1px solid ${T.inputBorder}`, borderRadius:6, color:T.text, fontSize:11, outline:'none', boxSizing:'border-box' }}/>}
            <ResponsiveContainer width="100%" height={80}>
              <PieChart><Pie data={catData} cx="50%" cy="50%" innerRadius={18} outerRadius={36} dataKey="value" paddingAngle={2} stroke="none">{catData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip formatter={v=>`${v}%`} contentStyle={{ background:T.tooltipBg, border:`1px solid ${T.border}`, borderRadius:8, fontSize:11 }}/></PieChart>
            </ResponsiveContainer>
            <div style={{ overflowY:'auto', flex:1, marginTop:6, maxHeight:200 }}>
              {filteredCats.map(c=>(
                <div key={c.name} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:5 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:5 }}><div style={{ width:6, height:6, borderRadius:'50%', background:c.color, flexShrink:0 }}/><span style={{ fontSize:10, color:T.textSub, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:110 }}>{c.name}</span></div>
          ...
          {verdictData.length>0&&(
            <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:'14px 12px' }}>
              <div style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1.5, color:T.textSub, marginBottom:10 }}>Verdicts</div>
              {verdictData.map(v=>{
                ...
                return(
                  <div key={v.name} style={{ marginBottom:8 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}><span style={{ fontSize:11, color:T.textSub }}>{label}</span><span style={{ fontSize:11, fontWeight:700, color:v.color }}>{v.value}</span></div>
                    <div style={{ height:5, background:T.surface, borderRadius:3, overflow:'hidden', border:`1px solid ${T.border}` }}><div style={{ height:'100%', width:`${barW}%`, background:v.color, borderRadius:3 }}/></div>
                  </div>
                );
              })}
            </div>
  ...
  return(
    <div style={{ overflowY:'auto', flex:1 }}>
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ fontSize:12, color:T.textMuted, marginTop:2 }}>User = no dashboard · Team = view dashboard · Admin = full access</div>
      </div>
      <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, overflow:'hidden' }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 100px 180px', gap:12, padding:'10px 16px', background:T.headerRow, borderBottom:`1px solid ${T.border}` }}>
          {['User','Role','Change Role'].map(h=><span key={h} style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:1, color:T.textMuted }}>{h}</span>)}
        </div>
        {users.map((u,i)=>{
          const letter=(u.username||'U')[0]?.toUpperCase();
          const isSaving=saving===u.id;
          return(
            <div key={u.id} style={{ display:'grid', gridTemplateColumns:'1fr 100px 180px', gap:12, padding:'12px 16px', alignItems:'center', borderBottom:i<users.length-1?`1px solid ${T.border}`:'none' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, minWidth:0 }}>
                ...
                <div><div style={{ fontSize:12, fontWeight:600, color:T.text }}>@{u.username||'unnamed'}</div><div style={{ fontSize:10, color:T.textMuted }}>{new Date(u.created_at).toLocaleDateString('en-US',{month:'short',year:'numeric'})}</div></div>
              </div>
              <span style={{ fontSize:12, color:ROLE_COLOURS[u.role], fontWeight:600 }}>{ROLE_LABELS[u.role]}</span>
              <div style={{ display:'flex', gap:4 }}>
                {Object.values(ROLES).map(r=>(
                  <button key={r} onClick={()=>changeRole(u.id,r)} disabled={u.role===r||isSaving} style={{ padding:'4px 10px', borderRadius:6, cursor:u.role===r?'default':'pointer', fontSize:10, fontWeight:600, transition:'all 0.15s', background:u.role===r?ROLE_COLOURS[r]+'22':T.surface, border:`1px solid ${u.role===r?ROLE_COLOURS[r]:T.border}`, color:u.role===r?ROLE_COLOURS[r]:T.textMuted, opacity:isSaving?0.5:1 }}>{ROLE_LABELS[r]}</button>
                ))}
              </div>
            </div>
          );
...
const ProfileEdit = ({ profile, onSaved, theme:T }) => {
  ...
  return !editing?(
    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
      <div style={{ flex:1 }}><div style={{ fontSize:11, color:T.textMuted, marginBottom:4 }}>Username</div><div style={{ fontSize:15, fontWeight:600, color:T.text }}>{profile?.username||'—'}</div></div>
      <button onClick={()=>{ setEditing(true); setInput(profile?.username||''); }} style={{ background:T.accent+'18', border:`1px solid ${T.accent}44`, color:T.accent, padding:'6px 14px', borderRadius:8, cursor:'pointer', fontSize:12, fontWeight:600 }}>Edit Username</button>
    </div>
  ):(
    <div>
      <div style={{ fontSize:11, color:T.textMuted, marginBottom:6 }}>New Username</div>
      <input value={input} onChange={e=>setInput(e.target.value)} placeholder="Enter username" style={{ width:'100%', padding:'8px 12px', background:T.inputBg, border:`1px solid ${T.inputBorder}`, borderRadius:8, color:T.text, fontSize:13, outline:'none', boxSizing:'border-box', marginBottom:10 }}/>
      <div style={{ display:'flex', gap:8 }}>
        <button onClick={save} disabled={loading} style={{ background:T.accentGrad, border:'none', color:'#fff', padding:'7px 18px', borderRadius:8, cursor:'pointer', fontSize:12, fontWeight:700 }}>{loading?'Saving…':'Save'}</button>
      ...
      <div style={{ position:'fixed', inset:0, background:T.overlay, display:'flex', alignItems:'center', justifyContent:'center', zIndex:1400, padding:'3%', boxSizing:'border-box' }} role="dialog" aria-modal="true">
        <div style={{ background:T.bg, backgroundImage:T.bgGradient, width:'100%', height:'100%', borderRadius:16, border:`1px solid ${T.border}`, boxShadow:T.shadow, padding:'20px 24px', position:'relative', display:'flex', flexDirection:'column', fontFamily:"'DM Sans',sans-serif", overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:18, paddingRight:36, flexShrink:0 }}>
            <div style={{ width:42, height:42, borderRadius:10, flexShrink:0, background:`linear-gradient(135deg,${T.accent},${T.accent}99)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, fontWeight:800, color:'#fff' }}>{avatarLetter}</div>
            <div>
            ...
            <div style={{ display:'flex', gap:3, background:T.surface, borderRadius:8, padding:3, border:`1px solid ${T.border}` }}>
              {tabs.map(t=>(
                <button key={t.id} onClick={()=>setTab(t.id)} style={{ background:activeTab===t.id?T.tabActive:'transparent', border:'none', color:activeTab===t.id?T.tabText:T.tabInactive, padding:'5px 14px', borderRadius:6, cursor:'pointer', fontSize:12, fontWeight:600, whiteSpace:'nowrap', transition:'all 0.15s' }}>{t.label}</button>
              ))}
            </div>
          </div>
