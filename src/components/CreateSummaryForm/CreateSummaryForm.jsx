...
const fetchFullRow = async (id) => {
  if (!id) return null;
  try {
    const { data, error } = await supabase.from("book_summaries").select("*").eq("id", id).maybeSingle();
    if (error) throw error;
    return data;
  } catch (err) { console.error("fetchFullRow error:", err); return null; }
};
...
const CreateSummaryForm = ({ onClose, onNewSummary, editingSummary = null }) => {
  ...
  useEffect(() => {
    if (!editingSummary?.id) { setInitData(editingSummary); return; }
    setInitLoading(true);
    ...
    return (
      <div className="modal-overlay">
        <div className="modal-content large" style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:300 }}>
          <div style={{ color:"#6b7280", fontSize:14 }}>Loading review…</div>
        </div>
      </div>
    );
  }
...
const CreateSummaryInner = ({ onClose, onNewSummary, editingSummary }) => {
  ...
  const buildPayload = useCallback((user, finalSlug, status) => ({
  ...
  const resolveSlug = async (base, excludeId = null) => {
    let finalSlug = base;
    try {
      let q = supabase.from("book_summaries").select("id").eq("slug", finalSlug);
  ...
  const _autoSave = useCallback(async () => {
    ...
    try {
      ...
      if (!user) { setAutoSaveStatus("error"); return; }
      let finalSlug = slug || slugify(title || `draft-${Date.now()}`, { lower:true, strict:true, replacement:"-" });
      const payload = buildPayload(user, finalSlug, "draft");
      if (draftId) {
  ...
  }, [title, author, description, summaryText, category, imageUrl, affiliateLink, affiliateType, youtubeUrl, tags, draftId, slug, buildSnapshot, buildPayload]);
  ...
  }, [title, author, description, summaryText, category, imageUrl, affiliateLink, affiliateType, youtubeUrl, tags, _autoSave, buildSnapshot]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!author.trim()) { setErrorMsg("Author / Reviewer is required."); return; }
    ...
    try {
      ...
      const { data } = await q.limit(1).maybeSingle();
      if (data) {
        setTitleDupeWarning(`⚠️ A published review called "${title.trim()}" already exists. Change the title before publishing.`);
        return;
      }
    } catch (_) {}
    ...
    try {
      ...
      if (!draftId) finalSlug = await resolveSlug(finalSlug);
      const payload = { ...buildPayload(user, finalSlug, "published"), category: publishCategory };
      const { error } = draftId
